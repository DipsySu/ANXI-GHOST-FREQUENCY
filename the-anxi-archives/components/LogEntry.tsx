...
const EraStyles: Record<Era, string> = {
  [Era.GOLDEN_AGE]: "border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.3)] bg-cyan-950/20 text-cyan-100",
  [Era.TURNING_POINT]: "border-red-600 shadow-[0_0_15px_rgba(220,38,38,0.3)] bg-red-950/20 text-red-100",
  [Era.WASTELAND]: "border-orange-700 shadow-[0_0_10px_rgba(194,65,12,0.2)] bg-amber-950/30 text-amber-100 font-serif-sc",
  [Era.GHOST_SIGNAL]: "border-emerald-900 shadow-[0_0_20px_rgba(16,185,129,0.1)] bg-black text-emerald-500 font-mono tracking-widest",
};

export const LogEntry: React.FC<LogEntryProps> = ({ log, onPlayAudio, isPlaying, lang }) => {
  const [glitchText, setGlitchText] = useState(log.content);
  const t = translations[lang];

  // Helper to translate backend signal values
  const getSignalText = (quality: string) => {
    if (quality.includes('良好') || quality.includes('High')) return t.sig_good;
    if (quality.includes('微弱') || quality.includes('Weak')) return t.sig_weak;
  ...
  // Helper to determine style based on quality
  const getSignalStyle = (quality: string) => {
    if (quality.includes('良好') || quality.includes('High') || quality.includes('Good')) return 'border-green-500 text-green-400 shadow-[0_0_10px_rgba(74,222,128,0.2)]';
    if (quality.includes('微弱') || quality.includes('Weak') || quality.includes('Low')) return 'border-yellow-500 text-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.2)]';
    // Default / Damaged
  ...
  // Ghost Signal Effect: Randomly corrupt text
  useEffect(() => {
  ...
  return (
    <div className={`relative p-6 border-l-4 mb-8 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4 ${styles}`}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 border-b border-white/10 pb-2">
        ...
        <div className="flex items-center gap-4 mt-2 md:mt-0">
          <span className={`text-xs px-2 py-0.5 rounded border ${signalStyle}`}>
            SIG: {signalText}
          </span>
          <button
          ...
          >
            {isPlaying ? <Square size={16} className="animate-pulse" /> : <Play size={16} />}
          </button>
        </div>
      </div>
      ...
      <div className={`text-lg leading-relaxed whitespace-pre-wrap ${log.era === Era.GHOST_SIGNAL ? 'blur-[0.5px]' : ''}`}>
      ...
      {log.era === Era.GHOST_SIGNAL && (
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none mix-blend-difference opacity-10 bg-[url('https://media.giphy.com/media/oEI9uBYSzLpBK/giphy.gif')] bg-cover"></div>
      )}
    </div>
  );
};